...
#if LIBCURL_VERSION_MAJOR < 8 || LIBCURL_VERSION_MINOR < 6
...
function build_for_arch() {
  ...
  ./configure --disable-shared --without-zlib --without-libpsl --enable-static --enable-ipv6 ${SSL_FLAG} --host="${HOST}" --prefix=${PREFIX} && make -j8 && make install
}
